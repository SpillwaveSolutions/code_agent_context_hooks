...
fn test_init_with_examples() {
    ...
    assert!(python_standards.exists(), "Python standards should be created");
...
fn test_debug_pretooluse_bash() {
    ...
    cch_cmd()
        .current_dir(temp_dir.path())
        .args(["debug", "PreToolUse", "--tool", "Bash", "--command", "echo test"])
        .assert()
        .success()
...
fn test_debug_detects_blocked_command() {
    ...
    cch_cmd()
        .current_dir(temp_dir.path())
        .args(["debug", "PreToolUse", "--tool", "Bash", "--command", "git push --force origin main"])
        .assert()
        .success()
...
fn test_debug_verbose_shows_rules() {
    ...
    cch_cmd()
        .current_dir(temp_dir.path())
        .args(["debug", "PreToolUse", "--tool", "Bash", "--command", "ls", "--verbose"])
        .assert()
        .success()
...
fn test_install_creates_settings_json() {
    ...
    assert!(content.contains("pre_tool_use"), "Should have pre_tool_use hook");
    assert!(content.contains("post_tool_use"), "Should have post_tool_use hook");
}
...
fn test_uninstall_removes_hooks() {
    ...
    let settings = temp_dir.path().join(".claude/settings.json");
    let content = fs::read_to_string(&settings).unwrap();
    assert!(!content.contains("pre_tool_use"), "Should not have pre_tool_use hook");
}
